...
const OrderHistoryPage = () => {
  ...
  useEffect(() => {
    if (userProfile?.branchId || userProfile?.branch?.id) {
      const branchId = userProfile?.branchId || userProfile?.branch?.id;
      dispatch(getOrdersByBranch({ 
        branchId, 
        cashierId: userProfile.id,
        paymentType: paymentType === "ALL" ? null : paymentType
      }));
    }
  }, [dispatch, userProfile, paymentType]);
  ...
  const filteredOrders = useMemo(() => {
    ...
    // We only keep search term filtering on the client for instant feedback.
    return orders.filter((order) => {
      const orderDate = new Date(order.createdAt);

      // Date filtering (Mobile/Offline support or as secondary filter)
      let dateMatch = false;
      if (dateFilter === "today") {
      ...
      <Card className="bg-black/20 backdrop-blur-lg border border-white/10 rounded-2xl">
        <CardContent className="p-6">
          <div className="flex flex-wrap gap-4 mb-6">
            ...
            <div className="flex gap-2">
              <Button
                variant={paymentType === "ALL" ? "default" : "outline"}
              ...
              {paymentMethods.map(method => (
                <Button
                  key={method.key}
                  variant={paymentType === method.key ? "default" : "outline"}
